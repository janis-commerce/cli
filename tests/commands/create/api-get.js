...
const {
	...
	builder,
	handler
} = require('../../../lib/commands/create/api-get');

describe('Commands', () => {

	describe('Create API Get', () => {
		...
		describe('command', () => {
			it('Should export the create-api-get command', () => {
				assert.strictEqual(command, 'create-api-get');
			});
		});
		...
		describe('handler', () => {
			...
			it('Should write and open all the files', async () => {
				...
				sinon.assert.callCount(fs.outputFile, 9);
				sinon.assert.callCount(childProcess.spawn, 9);
			});
			...
			it('Should open but not write existing files that should not be overriden', async () => {
				...
				sinon.assert.callCount(fs.pathExists, 4);
				sinon.assert.callCount(fs.outputFile, 6);
				sinon.assert.callCount(childProcess.spawn, 9);
			});
