...
module.exports.upperCamelCase = string => camelcase(string.charAt(0).toUpperCase() + string.slice(1));
...
module.exports.getValidationTestCase = (entity, modelName, fieldName, modelMethod) => {
	...
	return `		{
			...
			before: sinon => {
				sinon.spy(${modelName}.prototype, '${modelMethod}');
			},
			after: (response, sinon) => {
				sinon.assert.notCalled(${modelName}.prototype.${modelMethod});
			}
		},`;
};
...
module.exports.getAuthorizerFromAuth = auth => {

	if(auth === 'full')
		return 'FullAuthorizer';

	if(auth === 'core')
		return 'NoClientAuthorizer';
};
