...
module.exports = ({
	...
	const validationTestCases = fields
		.map(f => !ignoredFields[f] && getValidationTestCase(entity, modelName, f, 'update'))
		.filter(Boolean);
...
${validationTestCases.join('\n')}
		{
			...
			before: sinon => {
				sinon.stub(${modelName}.prototype, 'update')
					.rejects(new Error('Error updating'));
			},
			after: (response, sinon) => {
				sinon.assert.calledOnceWithExactly(${modelName}.prototype.update, { ...${entity}Formatted }, { id });
			}
		},
		{
			description: 'Should update the ${entity} with all the formatted fields',
			request: {
				data: { ...${entity} }
			},
			response: {
