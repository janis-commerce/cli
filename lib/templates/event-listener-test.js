...
module.exports = ({
	...
	const apiTitle = startcase(`${entity}-${event}-listener`);
	const apiName = apiTitle.replace(/ /g, '');

	const sourceRelativePath = path.relative(path.dirname(getTestFilePath(entity, event)), getSourceFilePath(entity, event));
